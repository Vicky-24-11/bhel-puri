...
export default function SellScreen() {
  ...
  const [auctionType, setAuctionType] = useState<"forward" | "reverse">(
    "forward",
  );
  ...
  const validateStep = (currentStep: number) => {
    ...
    if (currentStep === 3)
      return auctionType === "reverse" ? true : imageUris.length > 0;
    if (currentStep === 4)
    ...
    if (currentStep === 5) {
      ...
      const durationVal = isCustomDuration
        ? parseFloat(customDurationHours)
        : durationPreset;
  ...
  const handlePublish = async () => {
    if (!user) {
      Alert.alert(
        ...
        "Please log in to publish an auction.",
      );
      return;
    }
    ...
    try {
      ...
      const endsAt = new Date(
        startsAt.getTime() + durationVal * 60 * 60 * 1000,
      );
      ...
      for (const uri of imageUris) {
        if (Platform.OS === "web") {
          ...
          try {
            ...
            console.warn(
              "Compression failed for image, fallback to original:",
              compressErr,
            );
            processedUris.push(uri);
          }
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="flex-1">
        <View className="w-full max-w-2xl mx-auto px-5 pt-6 pb-12">
              ...
              <View className="gap-4 mb-4">
                ...
                >
                  ...
                  <View className="flex-1">
                    ...
                    <Text className="font-display text-xs text-brand-muted">
                      Buyers compete and the highest bid wins. Best for selling
                      items.
                    </Text>
                  </View>
                  {auctionType === "forward" && (
                ...
                >
                  ...
                  <View className="flex-1">
                    ...
                    <Text className="font-display text-xs text-brand-muted">
                      Sellers compete and the lowest offer wins. Best for
                      sourcing items.
                    </Text>
                  </View>
                  {auctionType === "reverse" && (
          ...
          {step === 2 && (
            <View className="gap-4">
              <Text className="text-lg font-display font-bold text-brand-text">
                ...
                {auctionType === "reverse" ? "buy request" : "item"}
              </Text>
                ...
                {categories.map((cat) => {
                  ...
                  return (
                    <Pressable
                      ...
                      className="w-[48%] p-4 rounded-2xl border flex-row items-center justify-between shadow-sm active:opacity-95"
                    >
                      <Text
                        className={`font-display text-sm font-semibold ${isSelected ? "text-brand-primary" : "text-brand-text"}`}
                      >
                        {cat.name}
                      </Text>
          ...
          {step === 3 && (
            <View className="gap-5">
              <Text className="text-lg font-display font-bold text-brand-text">
                Add reference or item photos{" "}
                {auctionType === "reverse" && "(Optional)"}
              </Text>
              ...
              >
                <Camera size={32} color="#FF6B35" className="mb-1" />
                ...
                <View className="flex-row flex-wrap gap-3 my-2">
                  {imageUris.map((uri, idx) => (
                    ...
                    >
                      ...
                      {/* Flag primary image */}
                      {idx === 0 && (
                        <View className="absolute bottom-0 left-0 right-0 bg-brand-primary py-1 items-center">
                          <Text className="text-[8px] font-display font-bold text-white uppercase">
                            Primary
                          </Text>
                        </View>
                      )}
          ...
          {step === 4 && (
            <View className="gap-4">
              ...
              <Input
                ...
                placeholder={
                  auctionType === "reverse"
                    ? "e.g. Looking for iPhone 15 Pro Max"
              ...
              <Input
                ...
                placeholder={
                  auctionType === "reverse"
                    ? "Detail the required item condition, specific model specs, or color preferences..."
                    : "Detail the product condition, age, inclusions, or any blemishes..."
                }
                value={description}
                ...
                helperText={
                  auctionType === "reverse"
                    ? "Provide at least 10 characters to inform sellers"
                    : "Provide at least 10 characters to inform buyers"
                }
              />
              ...
              <View>
                ...
                <View className="flex-row flex-wrap gap-2 mb-3">
                  {durationPresetOptions.map((opt) => {
                    const isActive =
                      !isCustomDuration && durationPreset === opt.value;
                    return (
                      <Pressable
                        key={opt.value}
                        onPress={() => {
                      ...
                      >
                        <Text
                          className={`text-xs font-display font-bold ${isActive ? "text-white" : "text-brand-text"}`}
                        >
                          {opt.label}
                        </Text>
                      </Pressable>
                  ...
                  >
                    <Text
                      className={`text-xs font-display font-bold ${isCustomDuration ? "text-white" : "text-brand-text"}`}
                    >
                      Custom Duration
                    </Text>
                  </Pressable>
          ...
          {step === 6 && (
            <View className="gap-5">
              <Text className="text-lg font-display font-bold text-brand-text">
                Preview your{" "}
                {auctionType === "reverse" ? "buy request" : "listing"}
              </Text>
              ...
              <View className="bg-white border border-stone-200 rounded-3xl overflow-hidden shadow-sm p-4 gap-4">
                ...
                <View className="gap-1.5 mt-2">
                  <View className="flex-row gap-2">
                    <Badge
                      ...
                      type={auctionType === "reverse" ? "warning" : "success"}
                      size="sm"
                    />
                    <Badge
                ...
                <View className="flex-row justify-between">
                  <View>
                    ...
                    <Text className="text-base font-display font-extrabold text-brand-text mt-0.5">
                      ₹{parseFloat(startingPrice).toLocaleString("en-IN")}
                    </Text>
                  </View>
                  <View className="items-end">
                    ...
                    <Text className="text-base font-display font-extrabold text-brand-text mt-0.5">
                      ₹{parseFloat(minIncrement).toLocaleString("en-IN")}
                    </Text>
                  </View>
                </View>
                ...
                {auctionType === "reverse" && minimumPrice !== "" && (
                  <View className="flex-row justify-between pt-1">
                    <View>
                      ...
                      <Text className="text-sm font-display font-extrabold text-brand-text mt-0.5">
                        ₹{parseFloat(minimumPrice).toLocaleString("en-IN")}
                      </Text>
                    </View>
                  </View>
                )}
              ...
              <View
                ...
                className="flex-row border border-brand-primary/20 p-4 rounded-2xl gap-3"
              >
                <Info size={20} color="#FF6B35" />
                <Text className="flex-1 text-[11px] font-display text-brand-muted leading-relaxed">
                  By clicking publish, this listing will go live immediately.
                  All details, pricing settings, and schedules are
                  authoritatively written to Supabase and cannot be modified
                  once live.
                </Text>
              </View>
              ...
              <View className="bg-white border border-stone-200 rounded-3xl p-4 gap-3.5 mt-1">
                ...
                >
                  <View
                    className={`w-5 h-5 rounded-lg border flex items-center justify-center ${confirmAuthorized ? "bg-brand-primary border-brand-primary" : "border-stone-300 bg-stone-50"}`}
                  >
                    {confirmAuthorized && <Check size={12} color="#FFFFFF" />}
                  </View>
                  <Text className="flex-1 text-[11px] font-display text-brand-text leading-relaxed">
                    I confirm that I am authorized to publish this request and
                ...
                >
                  <View
                    className={`w-5 h-5 rounded-lg border flex items-center justify-center ${confirmAuctionRules ? "bg-brand-primary border-brand-primary" : "border-stone-300 bg-stone-50"}`}
                  >
                    {confirmAuctionRules && <Check size={12} color="#FFFFFF" />}
                  </View>
                  <Text className="flex-1 text-[11px] font-display text-brand-text leading-relaxed">
                    I agree to the rules: listing details cannot be changed once
                    live, bids/offers are binding, and bidding on my own
                    listings is strictly forbidden.
                  </Text>
                </Pressable>
              </View>
              ...
              {publishing ? (
                <View className="py-4 items-center gap-2">
                  ...
                  <Text className="text-xs font-display text-brand-muted font-medium">
                    Publishing listing and uploading photos...
                  </Text>
                </View>
              ) : (
